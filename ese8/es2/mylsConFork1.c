...
int main (int argc, char** argv)
{
   ...
   int pidFiglio, status, ritorno;	/* per wait padre */
    ...
    if (argc != 2){
        printf("Errore: Necessario 1 parametro per %s e invece argc=%d\n", argv[0], argc); 
        exit(1);
    }

    //non devo controllare che il parametro sia un file o una directory, sarà compito dell'ls
	...
	else
	{
        ...
        if(ritorno != 255){
            ...
            printf("ERRORE - Il figlio con pid=%d ha ritornato %d\n", pidFiglio, ritorno);
        }
	}	
